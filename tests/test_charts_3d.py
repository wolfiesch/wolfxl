"""Sprint Μ-prime Pod-β′ (RFC-046 §11) — 3D / Surface / Stock / OfPie tests.

Covers construction + ``to_rust_dict()`` shape for the 8 new chart
families. End-to-end XML emit (``ws.add_chart`` → save → re-open) is
exercised once Pod-α′ has landed; until then these tests verify the
Python-side dict contract.
"""

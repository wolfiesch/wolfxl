...
2. **Python round-trip** — `tests/test_xlsb_reads.py` +
   `tests/test_xls_reads.py` cover values + formulas + sheet
   ordering; `test_bytes_input.py` covers path / BytesIO / bytes /
   file-like input across all three formats.
...
5. **Regression fixture** — `tests/fixtures/finance_q3.xlsb`
   (cached formulas + multi-sheet) and `tests/fixtures/legacy_erp.xls`
   (single sheet, dates + numbers + strings). Both checked in.

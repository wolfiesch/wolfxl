...
| `src/calamine_styled_backend.rs` | 3932 | Split reader extraction into styles, hyperlinks, comments, drawings, tables, conditional formatting, and validations modules. |
...
82. Worksheet structural operation docs and workbook sheet copy/move docs were
   rewritten into user-facing Google-style docs on 2026-04-29, removing stale
   RFC/sprint implementation notes without behavior changes.
...
93. Chart series emission, including series data labels, error bars,
...
99. Sheet-copy simple internal-part clones now share a helper in
...
103. Writer worksheet relationship-id allocation was centralized in
   `crates/wolfxl-writer/src/emit/sheet_rel_ids.rs` on 2026-04-29 so sheet
   rels, tables, hyperlinks, and drawing references share one ordering plan.
...
114. Modify-mode slicer queueing was split in
   `python/wolfxl/_workbook_patcher_flush.py` on 2026-04-29 so each slicer
   presentation serializes and queues through one helper while preserving the
   existing skip-on-error behavior.
...
117. Modify-mode sheet-setup queueing was split in
   `python/wolfxl/_workbook_patcher_flush.py` on 2026-04-29 so pending-state
   checks, setup payload construction, and patcher queueing are named helpers.
118. Write-mode worksheet setup, page-break, and autofilter flushing was split
   in `python/wolfxl/_worksheet_flush.py` on 2026-04-29 so native writer
   payload construction and defensive writer calls live in named helpers.
...
123. Rust save orchestration now has an explicit `SaveWorkspace` in
   `src/wolfxl/patcher_save.rs` to group the shared save-phase state:
   replacement patches, the part-id allocator, cloned table names, and local
   sheet blocks. This is the first low-risk step toward breaking up
   `XlsxPatcher::do_save` without changing the ordered phase calls.
124. Calamine styled-reader formatting helpers moved to
   `src/calamine_format_helpers.rs`, and array/data-table formula parsing
   moved to `src/calamine_styled_array_formulas.rs` on 2026-04-29. This
   removes pure helper islands from `src/calamine_styled_backend.rs` while
   preserving the PyO3 class surface and cache layout.
125. Source ZIP opening moved into `src/wolfxl/patcher_save.rs` on
...
128. Record-format carrier structs moved to `src/calamine_record_format.rs`
   on 2026-04-29 so raw-font overlays and compact record formatting no longer
   live inside the main styled backend.
129. Cell-format PyDict emitters moved to `src/calamine_style_dicts.rs` on
   2026-04-29 so font/fill/alignment/border dict population is shared outside
   the main styled backend.
130. Sheet-record emission options moved to `src/calamine_sheet_records.rs`
   on 2026-04-29, replacing the long internal boolean argument train before
   attempting a higher-risk record-builder extraction.
131. The diagonal-border carrier moved into `src/calamine_style_dicts.rs` on
   2026-04-29 so style-specific structs sit with the style dict helpers.
132. Column-letter index conversion moved into `src/calamine_value_helpers.rs`
   on 2026-04-29, leaving the main styled backend under 4,000 lines.
133. Sheet-record emission decisions plus formula/value PyDict population moved
   into `src/calamine_sheet_records.rs` on 2026-04-29, leaving the backend to
   provide style-cache callbacks and append the completed records.
134. Next helper candidate: continue splitting Rust save orchestration only

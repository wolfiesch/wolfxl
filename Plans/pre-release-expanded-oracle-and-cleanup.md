...
|---|---:|---|
| `src/wolfxl/mod.rs` | 4185 | Continue splitting patcher phases and save-path orchestration behind the same PyO3 surface. |
...
8. Next helper candidate: pivot/slicer phase extraction if the state boundary
   is clean, or switch to Python public API docstrings if the remaining Rust
   save phases look too coupled for another safe extraction.

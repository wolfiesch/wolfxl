...
| G08 | Threaded comments write+modify                                      | P1  | ∥ 🧠  | S2     | in-progress | Claude  | main | 2026-05-03 |           | Steps 2+3 landed (6082148, d7fb7b0): Python class layer + writer model/emit + Python→Rust plumbing. Save produces canonical OOXML (`xl/threadedComments/N.xml`, `xl/persons/personList.xml`, legacy placeholder + extLst-equivalent `tc=guid` author). openpyxl reads the file cleanly. Step 4 (reader) lands next; G08 will close once round-trip test in compat-oracle stops xfailing. |

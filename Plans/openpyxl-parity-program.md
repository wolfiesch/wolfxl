...
| G14 | CF stop-if-true, priority ordering, dxf integration                 | P2  | ⊥ 🤖  | S3     | landed      | Claude  | parity/g14-cf-stop-if-true-dxf | 2026-05-03 | 2026-05-03 | `CellIsRule(fill=PatternFill(...))` and `FormulaRule(fill=...)` accept the openpyxl `fill=` / `font=` / `border=` / `dxf=` kwargs and route them through `Rule.extra`; `_conditional_format_payload` (`python/wolfxl/_worksheet_flush.py`) translates a fill into a `format.bg_color` payload key so `dict_to_conditional_format` interns a matching `DxfRecord` via `StylesBuilder::intern_dxf` and stamps the resulting `dxfId` on the emitted `<cfRule>`. Two rules sharing a fill share one dxf slot. Explicit `rule.priority = N` rides through to a new `ConditionalRule.priority: Option<u32>` field; the emitter prefers the user value over the positional fallback. `Rule.dxf` openpyxl-shaped property exposes the `DifferentialStyle` shim back to introspecting callers. 8 tests in `tests/test_cf_stop_if_true.py` (stopIfTrue + default + explicit priority + ordering + dxf round-trip + dedup + no-dxf-no-fill + combined). Oracle probe `cf.stop_if_true_priority` flipped `partial -> supported`; pass rate 86.3% -> 88.2% (45/51). Full suite 2508 passed; cargo workspace clean. |
| G15 | Combination charts (mixed types, secondary axis)                    | P1  | ⊥ 🧠  | S4     | landed      | Claude  | parity/g15-combination-charts | 2026-05-03 | 2026-05-03 | `BarChart += LineChart` now emits a multi-family `<plotArea>` (RFC-069 §6) with a secondary `<c:valAx>` (right side, `crosses="max"`) when the secondary's `y_axis.axId` is distinct. Python: `ChartBase.to_rust_dict()` serialises `self._charts[1:]` under a new `secondary_charts` payload key; `_validate_at_emit` rejects empty-series secondaries, Pie/Doughnut secondaries, and same-kind/same-axId copy-paste shapes. Rust: new `Chart.secondary_charts: Vec<Chart>` parsed in `parse_chart_dict` and consumed by `emit_chart_xml` via `secondary_axis_pair()` + `emit_axis_as_secondary()`. Dual-emission strategy in `_flush_pending_charts` writes each family as its own standalone chartspace at the same anchor so openpyxl's reader (which collapses combos to a single returned chart) exposes both `BarChart` and `LineChart` on `ws._charts` — flipping the `charts_combination` compat-oracle probe `not_yet → supported`. 6 new tests in `tests/test_combination_charts.py` (shared axes / secondary axis / 3-family / 3 validation cases) plus 3 cargo unit tests. |
| G16 | Pivot chart per-point overrides                                     | P2  | ⊥ 🤖  | S4     | landed      | Claude  | parity/g16-pivot-chart-per-point | 2026-05-03 | 2026-05-03 | Per-point `<c:dPt>` round-trips through openpyxl on pivot-source charts. The Python `Series.dPt` -> `data_points` -> Rust `DataPoint` -> `<c:dPt><c:spPr>` bridge was already wired (RFC-046); G16 only had to register the oracle probe. New oracle probe `charts_pivot_chart_per_point` (bar + pivot_source + idx=1 red fill) and four focused tests in `tests/test_chart_per_point.py` (bar single, line two-points, multi-series, pivot+per-point). Spec entry flipped `partial -> supported`. |
| G17 | Pivot table mutation of existing pivots                             | P1  | ⊥ 🧠  | S5     | landed      | Claude  | parity/g17-pivot-mutation | 2026-05-03 | 2026-05-03 | RFC-070 Option B (source-range mutation only). New `crates/wolfxl-pivot/src/mutate.rs` parses & rewrites `pivotTable*.xml` + `pivotCacheDefinition*.xml` at byte level; `worksheetSource` ref/sheet attrs are surgically updated and `refreshOnLoad="1"` is stamped on shape-changing edits. PyO3 bridge in `src/wolfxl/patcher_pivot_parse.rs` + `src/wolfxl/patcher_pivot_edit.rs` exposes parsers and a save-time drain (Phase 2.5m-edit) sequenced after `apply_pivot_adds_phase`. Python: `python/wolfxl/pivot/_handle.py` (`PivotTableHandle` with mutator `source` setter) + `_load.py` (rels-graph walker with lazy materialization on `ws.pivot_tables`). Write-mode pivot construction now routes through a writer→tempfile→patcher two-phase save (`_save_write_mode_with_pivots`) so the openpyxl-shaped probe pattern (`wb=Workbook(); wb.add_pivot_cache; ws.add_pivot_table(pt,"A1"); wb.save`) works. Oracle probe `pivots.in_place_edit` flipped `not_yet → supported`; pass rate 90.4% → 92.3% (47/52 → 48/52). Full suite 2502 passed; cargo workspace 1282 tests green; external oracle (LibreOffice) preserves parts. 6 new tests in `tests/test_pivot_modify_existing.py` (5 pass, 1 xfail for missing fixture); 8 cargo integration tests in `crates/wolfxl-pivot/tests/parse_existing.rs`. Out of scope (deferred to v2): field placement mutation, filter mutation, aggregation function changes (RFC-070 §10). |
| G18 | External links (workbook-level collection + rels)                   | P1  | ⊥ 🧠  | S6     | landed      | Claude  | parity/g18-external-links | 2026-05-04 | 2026-05-04 | RFC-071 v1.0 — read-only inspection + opaque preservation. New pure-Rust parsers in `crates/wolfxl-reader/src/external_links.rs` (`parse_part`, `parse_rels`) wrapped by PyO3 helpers in `src/wolfxl/external_links.rs` and exposed as `wolfxl._rust.parse_external_link_part` / `parse_external_link_rels`. `wolfxl_rels::rt` gains `EXTERNAL_LINK` + `EXTERNAL_LINK_PATH` constants. Python `ExternalLink` / `ExternalFileLink` dataclasses live in `python/wolfxl/_external_links.py`; `Workbook._external_links` (with `external_links` alias) lazily walks `xl/_rels/workbook.xml.rels`, parses every `xl/externalLinks/externalLink{N}.xml` + sibling rels via the Rust helpers, and caches the resulting list. Save path needs no changes — `XlsxPatcher::rewrite_zip_phase` already round-trips unknown ZIP entries byte-for-byte, so `xl/externalLinks/**` survives modify-mode saves verbatim. Oracle probe `external_links.workbook_collection` flipped `not_yet → supported`; pass rate 92.3% → 94.2% (48/52 → 49/52). Full pytest 2508 passed (+6: 5 focused tests in `tests/test_external_links.py` + 1 oracle gain), cargo workspace green with 7 new unit + 6 new integration tests in `crates/wolfxl-reader/{src/external_links.rs,tests/external_links_parse.rs}`. Hand-built fixture at `tests/fixtures/external_links_basic.xlsx` (2735 bytes). Out of scope (deferred): authoring (`_external_links.append`), removal/rewrite, following the link target, updating `<sheetDataSet>` cached values. |
...
|----------------------------------|----------------------------------------------------------------------------------------|
| ``charts.combination``           | **Promoted to ``supported``** (2026-05-03, G15 landed) - multi-family `<plotArea>` plus dual-emission so openpyxl exposes both families on `ws._charts`. |
...
| G15 | RFC-069            | implemented | Combination charts (multi-family plotArea + secondary axis) |
| G17 | RFC-070            | implemented (Option B: source-range mutation only) | Pivot table mutation in modify mode |
| G18 | RFC-071            | implemented (v1.0: read-only inspection + opaque preservation) | External links - workbook-scoped collection + rels |
| G19 | RFC-072            | implemented | VBA inspection API (read-only) — `Workbook.vba_archive` returns raw bytes |
| G20 | RFC-073            | implemented | `write_only=True` streaming write mode            |

...
| G08 | Threaded comments write+modify                                      | P1  | ∥ 🧠  | S2     | in-progress | Claude  | main | 2026-05-03 | 2026-05-03 | Steps 2-4 landed: writer model/emit (6082148, d7fb7b0), Python→Rust write plumbing, reader for `xl/threadedComments/N.xml` + `xl/persons/personList.xml` with GUID-keyed reply-tree reassembly and `wb.persons` hydration. Compat-oracle `comments_threaded` flipped supported (39/50, 78%). Step 5 (modify-mode patcher entries) is the last residual before status moves to landed. |
...
|-----|--------------------|------------|---------------------------------------------------|
| G08 | RFC-068            | implemented (write+read) | Threaded comments write+modify          |

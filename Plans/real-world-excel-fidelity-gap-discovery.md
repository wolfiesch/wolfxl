...
| Current evidence bundle | `Plans/ooxml-current-evidence-bundle.json` plus `scripts/audit_ooxml_evidence_bundle.py --strict` pins the current generated report artifacts, records each producer command, and verifies expected readiness/counts across all-evidence coverage, interactive evidence, corpus diversity, gap radar, and recursive SynthGL side evidence | Latest bundle audit `/tmp/wolfxl-current-evidence-bundle-audit.json`: `ready=true`, 7 reports, 7 producer commands, 0 issues | This prevents stale or origin-unclear `/tmp` report references from silently supporting completion claims. It still verifies generated artifacts, not a permanent corpus expansion by itself |
...
| Chart/style/color dependency preservation | Chart, chart-sheet, chart-style, style/theme fingerprints plus render reports | Coverage report: `chart_style_color_preservation` clear, 5 fixtures, no missing evidence. Engine-specific probe `/tmp/wolfxl-coverage-excel-render-required-probe.json` reports missing `render_no_op_pass:excel` | Covered for pinned corpus; Excel-rendered pixels are now a machine-checkable missing evidence class |
...
| Excel-rendered output evidence | `scripts/audit_ooxml_fidelity_coverage.py tests/fixtures/external_oracle --strict --require-render --require-render-engine excel --report /tmp/wolfxl-ooxml-fidelity-mutations-p1-all-final.json --render-report /tmp/wolfxl-render-noop-p1-all-final.json` plus `scripts/run_ooxml_render_compare.py ... --render-engine excel` | Latest coverage probe `/tmp/wolfxl-coverage-excel-render-required-probe.json`: `ready=False`; LibreOffice render reports are correctly excluded when `excel` is required. Latest local Excel producer probe `/tmp/wolfxl-render-excel-simple-probe.json`: `failure_count=1`; Excel did not emit a non-empty PDF for the no-op saved workbook | Not covered yet; this is now an explicit red gate and producer blocker rather than a prose-only caveat |
...
| Evidence bundle freshness | `scripts/audit_ooxml_evidence_bundle.py Plans/ooxml-current-evidence-bundle.json --strict` | Latest run `/tmp/wolfxl-current-evidence-bundle-audit.json`: `ready=True`, 7 report artifacts verified, 7 producer commands recorded, 0 issues | Stronger provenance over the current generated evidence set; still dependent on generated reports being refreshed when fixtures or gates change |
...
- The repo should not claim: **no real-world Excel fidelity gaps exist.** That
  ...
  click-level slicer/timeline interaction automation, and more adversarial
  feature-specific mutations than the current pack can provide.
...
1. Add a larger external workbook corpus sweep with provenance buckets:
   ...
   workbooks.
2. Make the Microsoft Excel render producer pass for a small representative
   subset, then add it to the coverage bundle with
   `--require-render-engine excel`. The current `--render-engine excel` path
   exists but is red locally because Excel did not emit a PDF for the first
   no-op saved probe workbook.

...
| Interactive evidence audit | `scripts/audit_ooxml_interactive_evidence.py` inventories high-risk Excel behaviors that package/render/app-open checks cannot prove: slicer selection state, timeline selection state, pivot refresh/filter state, external-link update prompts, macro project presence, and embedded-control openability. `scripts/run_ooxml_interactive_probe.py` currently implements Microsoft Excel macro-project and embedded-control presence probes | Current strict run over the 22-fixture external-oracle pack with `/tmp/wolfxl-interactive-probe-macro-excel.json` and `/tmp/wolfxl-interactive-probe-embedded-control-excel.json` is still intentionally red: `/tmp/wolfxl-interactive-evidence-external-oracle-macro-control.json` clears macro project presence and embedded-control openability, while slicer selection, timeline selection, pivot refresh, and external-link prompt/cache behavior still lack passing interactive probe reports | This is the current highest-confidence blocker before any stronger "no known interactive gaps" claim. It prevents app-open smoke from being mistaken for interactive behavior proof |
...
2. Extend the mutation runner beyond safe edits:
   ...
   - Latest interactive-evidence audit slice:
     ...
     external-oracle output with the first probe report at
     `/tmp/wolfxl-interactive-evidence-external-oracle-macro-control.json` is
     intentionally not ready. `scripts/run_ooxml_interactive_probe.py` clears
     ...
     and verifying embedded/control OOXML parts remain present. Slicer
     selection state, timeline selection state, pivot refresh state, and
     external-link update prompt behavior still have applicable fixtures but no
     passing interactive probe reports.
...
| Interactive behavior evidence | `scripts/audit_ooxml_interactive_evidence.py tests/fixtures/external_oracle --report /tmp/wolfxl-interactive-probe-macro-excel.json --report /tmp/wolfxl-interactive-probe-embedded-control-excel.json --strict` | Latest run: `ready=False`; macro project presence is clear on `real-excel-macro-basic.xlsm`, and embedded-control openability is clear on `umya-control-props-basic.xlsx` plus `real-excel-control-props-basic.xlsx`. Applicable fixture candidates remain missing passing probes for slicer selection, timeline selection, pivot refresh, and external-link prompt/cache behavior | Partially covered. This is the clearest remaining evidence gap after package/render/app-open gates |

...
| Current evidence bundle | `Plans/ooxml-current-evidence-bundle.json` plus `scripts/audit_ooxml_evidence_bundle.py --strict` pins the current generated report artifacts, records each producer command, and verifies expected readiness/counts across all-evidence coverage, interactive evidence, Excel UI-interaction evidence, Excel-render full-pack no-op and intentional evidence, hardened Excel app-open evidence, corpus diversity, gap radar, recursive SynthGL side evidence, umya issue-corpus side evidence, public Microsoft sample evidence, and the Contoso PowerPivot plus shared-slicer sidecar evidence | Latest bundle audit `/tmp/wolfxl-current-evidence-bundle-audit-20260508-slicer-timeline-ui.json`: `ready=true`, 64 reports, 64 producer commands, 0 issues | This prevents stale or origin-unclear `/tmp` report references from silently supporting completion claims. It still verifies generated artifacts, not a permanent corpus expansion by itself |
...
1. Extend `scripts/audit_ooxml_fidelity.py` from first-pass semantic fingerprints
   ...
   - Still needed: additional structural/adversarial intentional-edit
     Excel-rendered smoke, deeper slicer/timeline item/date-range UI
     validation, and broader real-file corpus diversity.
...
| Evidence bundle freshness | `scripts/audit_ooxml_evidence_bundle.py Plans/ooxml-current-evidence-bundle.json --strict` | Latest run `/tmp/wolfxl-current-evidence-bundle-audit-20260508-slicer-timeline-ui.json`: `ready=True`, 64 report artifacts verified, 64 producer commands recorded, 0 issues | Stronger provenance over the current generated evidence set; still dependent on generated reports being refreshed when fixtures or gates change |
...
- The repo should not claim: **no real-world Excel fidelity gaps exist.** That
  ...
  slicer/timeline item/date-range interaction automation, and more adversarial mutations than
...
3. Continue expanding click-level interactive UI automation beyond the current
   `ooxml_state_presence` probes. Macro prompt handling, pivot refresh, slicer
   shape selection, and timeline shape selection now have pinned UI-interaction
   artifacts; slicer item choices, timeline date-range choices, external-link
   prompt reproduction under a prompt-forcing setup, and embedded-control
   click/open behavior remain open.

...
| Interactive evidence audit | `scripts/audit_ooxml_interactive_evidence.py` inventories high-risk Excel behaviors that package/render/app-open checks cannot prove: slicer selection state, timeline selection state, pivot refresh/filter state, external-link update prompts, macro project presence, and embedded-control openability. `scripts/run_ooxml_interactive_probe.py` currently implements the narrow Microsoft Excel macro-project presence probe | Current strict run over the 22-fixture external-oracle pack with `/tmp/wolfxl-interactive-probe-macro-excel.json` is still intentionally red: `/tmp/wolfxl-interactive-evidence-external-oracle-macro.json` clears macro project presence on `real-excel-macro-basic.xlsm`, while slicer, timeline, pivot refresh, external-link prompt/cache behavior, and embedded-control openability still lack passing interactive probe reports | This is the current highest-confidence blocker before any stronger "no known interactive gaps" claim. It prevents app-open smoke from being mistaken for interactive behavior proof |
...
2. Extend the mutation runner beyond safe edits:
   ...
   - Latest interactive-evidence audit slice:
     ...
     `/tmp/wolfxl-interactive-evidence-external-oracle-macro.json` is
     ...
     Microsoft Excel and verifying `xl/vbaProject.bin` remains present. Slicer
     selection state, timeline selection state, pivot refresh state,
     external-link update prompt behavior, and embedded-control openability
     still have applicable fixtures but no passing interactive probe reports.
...
| Interactive behavior evidence | `scripts/audit_ooxml_interactive_evidence.py tests/fixtures/external_oracle --report /tmp/wolfxl-interactive-probe-macro-excel.json --strict` | Latest run: `ready=False`; macro project presence is clear on `real-excel-macro-basic.xlsm`. Applicable fixture candidates remain missing passing probes for slicer selection, timeline selection, pivot refresh, external-link prompt/cache behavior, and embedded-control openability | Partially covered. This is the clearest remaining evidence gap after package/render/app-open gates |

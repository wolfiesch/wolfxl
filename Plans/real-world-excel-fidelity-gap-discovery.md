...
| Coverage evidence audit | `scripts/audit_ooxml_fidelity_coverage.py` maps fixtures plus mutation, render, and app reports to the evidence standard: external-tool fixture, real Excel fixture, structural mutation pass, no-op render pass, intentional mutation render pass, source app-open pass, and intentional app-open pass. It records concrete feature keys, requires slicer/timeline evidence separately from pivot-table evidence, rejects `--strict` runs that omit mutation reports, and supports recursive discovery, workbook application provenance fallback, optional emerging-surface gates, render/app required gates, and engine-specific render requirements | The current regenerated all-evidence coverage report is `/tmp/wolfxl-coverage-all-evidence-current-code-plus-excel-powerpivot.json`: `ready=true` over 22 fixtures, 13 surfaces, 4 mutation reports, 6 render reports, and 8 app reports. The Excel-render full-pack coverage report with marker-cell, copy-first-sheet, move-formula-range, and add-data-validation intentional smoke `/tmp/wolfxl-coverage-excel-render-full-pack-with-structural-formula-validation-intentional.json` is also `ready=true`: 22 fixtures, `render_engine_required=excel`, 5 Excel-render reports, 0 missing surfaces. A recursive live SynthGL coverage run maps 32 nested Excel-authored workbooks to the latest 160-result mutation report and now tracks optional Python-in-Excel and sheet-metadata surfaces when present | It audits evidence presence and no-op/intentional renderability. It now has full-pack Microsoft Excel-rendered no-op, marker-cell intentional, copy-first-sheet structural intentional, move-formula-range structural intentional, and add-data-validation feature-add evidence, but still does not prove click-level structural visual equivalence after every intentional edit, click-level interactive behavior, or broader real-file corpus coverage |
...
| Current evidence bundle | `Plans/ooxml-current-evidence-bundle.json` plus `scripts/audit_ooxml_evidence_bundle.py --strict` pins the current generated report artifacts, records each producer command, and verifies expected readiness/counts across all-evidence coverage, interactive evidence, Excel-render full-pack no-op and intentional evidence, hardened Excel app-open evidence, corpus diversity, gap radar, and recursive SynthGL side evidence | Latest bundle audit `/tmp/wolfxl-current-evidence-bundle-audit.json`: `ready=true`, 17 reports, 17 producer commands, 0 issues | This prevents stale or origin-unclear `/tmp` report references from silently supporting completion claims. It still verifies generated artifacts, not a permanent corpus expansion by itself |
...
1. Extend `scripts/audit_ooxml_fidelity.py` from first-pass semantic fingerprints
   ...
   - Still needed: additional structural/adversarial intentional-edit
     Excel-rendered smoke, interactive slicer/timeline state validation, and
     broader real-file corpus diversity.
2. Extend the mutation runner beyond safe edits:
   ...
   - Latest intentional-render slice: `scripts/run_ooxml_render_compare.py`
     ...
     move-formula-range, and add-data-validation mutations with 88 combined
     results and 0 failures, using page sampling for large exports. An
     exploratory add-conditional-formatting render sweep currently fails Excel
     PDF export on 3 openpyxl-authored fixtures. After hardening app-smoke to
     reject unrelated active workbooks, the same mutation also exposes 6 Excel
     openability failures, so that report is not pinned as positive evidence
     yet.
   ...
   - Latest interactive-evidence audit slice:
     ...
     present. Slicer selection state is clear for 4 slicer-bearing fixtures,
     and timeline selection state is clear for `real-excel-timeline-slicer.xlsx`,
     by opening them in Microsoft Excel and verifying slicer/timeline OOXML
     state remains present. This is a current openability/state-presence gate,
     not full click-level selection automation.
...
| Chart/style/color dependency preservation | Chart, chart-sheet, chart-style, style/theme fingerprints plus render reports | Coverage report: `chart_style_color_preservation` clear, 5 fixtures, no missing evidence. Engine-specific Excel render coverage `/tmp/wolfxl-coverage-excel-render-full-pack-with-structural-formula-validation-intentional.json` is also clear | Covered for pinned corpus with full-pack Excel-rendered no-op, marker-cell intentional, copy-first-sheet structural intentional, move-formula-range structural intentional, and add-data-validation evidence; additional feature-specific intentional Excel rendering remains stronger future evidence |
...
| Excel-rendered output evidence | `scripts/run_ooxml_render_compare.py ... --render-engine excel` plus `scripts/audit_ooxml_fidelity_coverage.py ... --require-render --require-render-engine excel` | Latest raw Excel-render reports: `/tmp/wolfxl-render-excel-full-pack.json` has 22 no-op results and 0 failures; `/tmp/wolfxl-render-excel-intentional-marker-full-pack.json` has 22 marker-cell mutation results and 0 failures; `/tmp/wolfxl-render-excel-intentional-copy-sheet-full-pack.json` has 22 copy-first-sheet structural mutation results and 0 failures; `/tmp/wolfxl-render-excel-intentional-move-formula-range-full-pack.json` has 22 move-formula-range structural mutation results and 0 failures; `/tmp/wolfxl-render-excel-intentional-add-data-validation-full-pack.json` has 22 add-data-validation feature-add results and 0 failures. Latest coverage report `/tmp/wolfxl-coverage-excel-render-full-pack-with-structural-formula-validation-intentional.json`: `ready=True`, 22 fixtures audited, 5 Excel render reports, 0 missing surfaces. The producer stages files in Excel's sandbox container to avoid macOS `Grant File Access` prompts. Exploratory add-conditional-formatting render sweep fails Excel PDF export on 3 openpyxl-authored fixtures; hardened Excel app-smoke for the same mutation fails 6 fixtures and remains an unresolved follow-up | Covered as full-pack no-op plus marker-cell intentional, copy-first-sheet structural intentional, move-formula-range structural intentional, and add-data-validation Excel-rendered evidence across the current surface matrix; still not exhaustive feature-specific intentional-edit visual equivalence |
| Interactive behavior evidence | `scripts/audit_ooxml_interactive_evidence.py tests/fixtures/external_oracle --report /tmp/wolfxl-interactive-probe-macro-excel.json --report /tmp/wolfxl-interactive-probe-embedded-control-excel.json --report /tmp/wolfxl-interactive-probe-external-link-excel.json --report /tmp/wolfxl-interactive-probe-pivot-excel.json --report /tmp/wolfxl-interactive-probe-slicer-excel.json --report /tmp/wolfxl-interactive-probe-timeline-excel.json --strict` | Latest run `/tmp/wolfxl-interactive-evidence-external-oracle-all.json`: `ready=True`; all six current probe classes clear | Covered for current Excel openability/state-presence probes; future click-level slicer/timeline interaction automation would be stronger |
...
| Evidence bundle freshness | `scripts/audit_ooxml_evidence_bundle.py Plans/ooxml-current-evidence-bundle.json --strict` | Latest run `/tmp/wolfxl-current-evidence-bundle-audit.json`: `ready=True`, 17 report artifacts verified, 17 producer commands recorded, 0 issues | Stronger provenance over the current generated evidence set; still dependent on generated reports being refreshed when fixtures or gates change |
...
- The repo should not claim: **no real-world Excel fidelity gaps exist.** That
  ...
  structural intentional-edit Excel-rendered smoke, click-level
  slicer/timeline interaction automation, and more adversarial mutations than
...
2. Extend structural intentional mutation Excel render-smoke beyond
   ...
   relationship-preserving edits; separately investigate why
   `add_conditional_formatting` causes Excel repair/open failures on 6
   fixtures under the hardened app-smoke and Excel PDF export failures on 3
   openpyxl-authored fixtures.

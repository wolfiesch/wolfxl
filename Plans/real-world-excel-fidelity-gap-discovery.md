...
| Coverage evidence audit | `scripts/audit_ooxml_fidelity_coverage.py` maps fixtures plus mutation, render, and app reports to the evidence standard: external-tool fixture, real Excel fixture, structural mutation pass, no-op render pass, intentional mutation render pass, source app-open pass, and intentional app-open pass. It records concrete feature keys, requires slicer/timeline evidence separately from pivot-table evidence, rejects `--strict` runs that omit mutation reports, and supports recursive discovery, workbook application provenance fallback, optional emerging-surface gates, render/app required gates, and engine-specific render requirements | The current regenerated all-evidence coverage report is `/tmp/wolfxl-coverage-all-evidence-current-code-plus-excel-powerpivot.json`: `ready=true` over 22 fixtures, 13 surfaces, 4 mutation reports, 6 render reports, and 8 app reports. The Excel-render full-pack coverage report `/tmp/wolfxl-coverage-excel-render-full-pack.json` is also `ready=true`: 22 fixtures, `render_engine_required=excel`, 1 Excel-render report, 0 missing surfaces. A recursive live SynthGL coverage run maps 32 nested Excel-authored workbooks to the latest 160-result mutation report and now tracks optional Python-in-Excel and sheet-metadata surfaces when present | It audits evidence presence and no-op/intentional renderability. It now has full-pack Microsoft Excel-rendered no-op evidence, but still does not prove full visual semantic equivalence after every intentional edit, click-level interactive behavior, or broader real-file corpus coverage |
...
| Current evidence bundle | `Plans/ooxml-current-evidence-bundle.json` plus `scripts/audit_ooxml_evidence_bundle.py --strict` pins the current generated report artifacts, records each producer command, and verifies expected readiness/counts across all-evidence coverage, interactive evidence, Excel-render full-pack evidence, corpus diversity, gap radar, and recursive SynthGL side evidence | Latest bundle audit `/tmp/wolfxl-current-evidence-bundle-audit.json`: `ready=true`, 9 reports, 9 producer commands, 0 issues | This prevents stale or origin-unclear `/tmp` report references from silently supporting completion claims. It still verifies generated artifacts, not a permanent corpus expansion by itself |
...
1. Extend `scripts/audit_ooxml_fidelity.py` from first-pass semantic fingerprints
   ...
   - Still needed: intentional-edit Excel-rendered pixel comparison,
...
| Chart/style/color dependency preservation | Chart, chart-sheet, chart-style, style/theme fingerprints plus render reports | Coverage report: `chart_style_color_preservation` clear, 5 fixtures, no missing evidence. Engine-specific Excel render coverage `/tmp/wolfxl-coverage-excel-render-full-pack.json` is also clear | Covered for pinned corpus with full-pack Excel-rendered no-op evidence; intentional-edit Excel rendering remains stronger future evidence |
...
| Excel-rendered output evidence | `scripts/run_ooxml_render_compare.py ... --render-engine excel` plus `scripts/audit_ooxml_fidelity_coverage.py ... --require-render --require-render-engine excel` | Latest raw Excel-render report `/tmp/wolfxl-render-excel-full-pack.json`: `render_engine=excel`, 22 results, 0 failures. Latest coverage report `/tmp/wolfxl-coverage-excel-render-full-pack.json`: `ready=True`, 22 fixtures audited, 1 Excel render report, 0 missing surfaces. The producer stages files in Excel's sandbox container to avoid macOS `Grant File Access` prompts | Covered as full-pack no-op Excel-rendered evidence across the current surface matrix; not yet intentional-edit visual equivalence |
...
- The repo should not claim: **no real-world Excel fidelity gaps exist.** That
  would require broader corpus diversity, intentional-edit Excel-rendered
...
1. Add a larger external workbook corpus sweep with provenance buckets:
   ...
   workbooks.
2. Add intentional mutation Excel render-smoke for the highest-risk fixtures, now that the full-pack no-op Excel render sweep is green.

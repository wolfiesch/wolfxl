...
| Excel-rendered output evidence | `scripts/audit_ooxml_fidelity_coverage.py tests/fixtures/external_oracle --strict --require-render --require-render-engine excel --report /tmp/wolfxl-ooxml-fidelity-mutations-p1-all-final.json --render-report /tmp/wolfxl-render-noop-p1-all-final.json` | Latest probe `/tmp/wolfxl-coverage-excel-render-required-probe.json`: `ready=False`; LibreOffice render reports are correctly excluded when `excel` is required | Not covered yet; this is now an explicit red gate rather than a prose-only caveat |

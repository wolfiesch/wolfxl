...
2. Extend the mutation runner beyond safe edits:
   ...
   - Latest intentional-render slice: `scripts/run_ooxml_render_compare.py`
     ...
     PDF export on 3 openpyxl-authored fixtures even though a clean Excel
     openability smoke passes all 22 fixtures, so that report is not pinned as
     positive evidence yet.
...
| Evidence bundle freshness | `scripts/audit_ooxml_evidence_bundle.py Plans/ooxml-current-evidence-bundle.json --strict` | Latest run `/tmp/wolfxl-current-evidence-bundle-audit.json`: `ready=True`, 14 report artifacts verified, 14 producer commands recorded, 0 issues | Stronger provenance over the current generated evidence set; still dependent on generated reports being refreshed when fixtures or gates change |
...
2. Extend structural intentional mutation Excel render-smoke beyond
   ...
   `add_conditional_formatting` opens cleanly in Excel but fails Excel PDF
   export on 3 openpyxl-authored fixtures.

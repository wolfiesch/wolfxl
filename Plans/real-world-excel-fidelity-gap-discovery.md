...
| Interactive evidence audit | `scripts/audit_ooxml_interactive_evidence.py` inventories high-risk Excel behaviors that package/render/app-open checks cannot prove: slicer selection state, timeline selection state, pivot refresh/filter state, external-link update prompts, macro project presence, and embedded-control openability. `scripts/run_ooxml_interactive_probe.py` currently implements Microsoft Excel macro-project, embedded-control, external-link prompt/cache, and pivot presence probes | Current strict run over the 22-fixture external-oracle pack with macro, embedded-control, external-link, and pivot probe reports is still intentionally red: `/tmp/wolfxl-interactive-evidence-external-oracle-macro-control-link-pivot.json` clears macro project presence, embedded-control openability, external-link prompt/cache behavior, and pivot refresh-state presence, while slicer and timeline selection still lack passing interactive probe reports | This is the current highest-confidence blocker before any stronger "no known interactive gaps" claim. It prevents app-open smoke from being mistaken for interactive behavior proof |
...
2. Extend the mutation runner beyond safe edits:
   ...
   - Latest interactive-evidence audit slice:
     ...
     external-oracle output with the current probe reports at
     `/tmp/wolfxl-interactive-evidence-external-oracle-macro-control-link-pivot.json` is
     ...
     present. Slicer selection state and timeline selection state still have
     applicable fixtures but no passing interactive probe reports.
...
| Interactive behavior evidence | `scripts/audit_ooxml_interactive_evidence.py tests/fixtures/external_oracle --report /tmp/wolfxl-interactive-probe-macro-excel.json --report /tmp/wolfxl-interactive-probe-embedded-control-excel.json --report /tmp/wolfxl-interactive-probe-external-link-excel.json --report /tmp/wolfxl-interactive-probe-pivot-excel.json --strict` | Latest run: `ready=False`; macro project presence, embedded-control openability, external-link prompt/cache behavior, and pivot refresh-state presence are clear. Applicable fixture candidates remain missing passing probes for slicer selection and timeline selection | Partially covered. This is the clearest remaining evidence gap after package/render/app-open gates |

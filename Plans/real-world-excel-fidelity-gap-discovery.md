...
2. Extend the mutation runner beyond safe edits:
   ...
   - Latest adjacent issue-corpus mutation slice: the 61-workbook
     ...
     add-conditional-formatting sweep with 183 results and 0 failures. The
     add-conditional-formatting mutation intentionally adds one `<dxf>` style
     record, so the runner now treats style/theme drift as expected only when
     `styles.xml` is otherwise identical after removing `<dxfs>` and the
     `<dxf>` count increases by exactly one.
...
| Umya issue-corpus diversity and quick mutation smoke | `scripts/audit_ooxml_corpus_buckets.py /Users/wolfgangschoenberger/Projects/umya-spreadsheet/tests/test_files --json`; `scripts/run_ooxml_fidelity_mutations.py ... --mutation no_op --mutation marker_cell --mutation add_conditional_formatting`; `scripts/audit_ooxml_fidelity_coverage.py ... --report /tmp/wolfxl-ooxml-fidelity-mutations-umya-quick.json` | Latest bucket run: 63 workbooks with 53 Excel-authored, 9 external-tool-authored, 2 macro/VBA, 2 external-link, 6 chart, 21 drawing/comment/media, 21 embedded/control, and 2 sheet-metadata examples; missing PowerPivot and slicer/timeline buckets. Latest quick mutation run: 183 results, 0 failures. Coverage side report: 61 fixtures, style/theme clear, sheet metadata clear, OOXML extension payload clear, intentionally `ready=false` as a full gate | Adds breadth and mutation confidence over an adjacent issue corpus; still needs richer structural mutations, render/app smoke, PowerPivot, and slicer/timeline examples before it can behave like a complete corpus |
...
| Evidence bundle freshness | `scripts/audit_ooxml_evidence_bundle.py Plans/ooxml-current-evidence-bundle.json --strict` | Latest run `/tmp/wolfxl-current-evidence-bundle-audit.json`: `ready=True`, 23 report artifacts verified, 23 producer commands recorded, 0 issues | Stronger provenance over the current generated evidence set; still dependent on generated reports being refreshed when fixtures or gates change |

...
| Current evidence bundle | `Plans/ooxml-current-evidence-bundle.json` plus `scripts/audit_ooxml_evidence_bundle.py --strict` pins the current generated report artifacts, records each producer command, and verifies expected readiness/counts across all-evidence coverage, interactive evidence, Excel UI-interaction evidence, Excel-render full-pack no-op and intentional evidence, hardened Excel app-open evidence, corpus diversity, gap radar, recursive SynthGL side evidence, umya issue-corpus side evidence, public Microsoft sample evidence, and the Contoso PowerPivot plus shared-slicer sidecar evidence | Latest bundle audit `/tmp/wolfxl-current-evidence-bundle-audit-20260508-axis-delete.json`: `ready=true`, 78 reports, 78 producer commands, 0 issues | This prevents stale or origin-unclear `/tmp` report references from silently supporting completion claims. It still verifies generated artifacts, not a permanent corpus expansion by itself |
...
2. Extend structural intentional mutation Excel render-smoke beyond the current
   `copy_first_sheet`, `move_formula_range`, `delete_first_row`,
   `delete_first_col`, `add_remove_chart`, and `retarget_external_links`
   slices into higher-risk feature-specific edits such as pivot/slicer
   structural edits and other external-link relationship-preserving edits.

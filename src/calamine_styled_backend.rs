...
use crate::calamine_format_helpers::{
    border_style_str, color_to_hex, h_align_str, is_default_font_size, openpyxl_builtin_num_fmt,
    strip_excel_padding, underline_str, v_align_str,
};
...
use crate::calamine_value_helpers::{
    data_is_formula_text, data_type_name, is_uncached_formula_value, map_error_formula,
    map_error_value, row_col_to_a1, update_bounds, update_dimensions,
};
...
impl CalamineStyledBook {
    ...
    pub fn read_sheet_records(
        ...
        if cell_range.is_none() && !include_empty {
            ...
            let mut cells: Vec<(u32, u32, Option<Data>)> = Vec::new();
            if let Some(range) = self.range_cache.get(sheet) {
...
impl CalamineStyledBook {
    ...
    fn ensure_value_caches(&mut self, sheet: &str) -> PyResult<()> {
        ...
        let afmap = parse_array_formulas_from_sheet_xml(xml)?;

...
pub(super) fn apply_external_links_drop_phase(
    ...
    let mut workbook_rels =
        match ooxml_util::zip_read_to_string_opt(zip, "xl/_rels/workbook.xml.rels")? {
            Some(s) => wolfxl_rels::RelsGraph::parse(s.as_bytes())
                .map_err(|e| PyIOError::new_err(format!("workbook rels parse: {e}")))?,
            None => return Ok(()),
        };
    ...
    patcher
        .rels_patches
        .insert("xl/_rels/workbook.xml.rels".to_string(), workbook_rels);

...
_Last rendered: **2026-05-05** from [`docs/migration/_compat_spec.py`](_compat_spec.py). Do not edit this file by hand; run `python scripts/render_compat_matrix.py`._
...
| `cross-sheet calc-chain ordering, deleted-cell pruning` | `cross-sheet calc-chain ordering, deleted-cell pruning` | ✅ Supported |  | Rebuild scans post-mutation sheet XML across tab order, prunes stale refs, and preserves a source calcChain extLst when emitting a non-empty chain. |
...
| `Slicer outside pivot context (table-driven, etc.)` | `Slicer outside pivot context (table-driven, etc.)` | ⛔ Out of Scope | G21 | openpyxl 3.1.5 exposes no public slicer modules/classes or table-driven slicer authoring API; tracked as a possible wolfxl-extra, not an openpyxl parity gap. |
